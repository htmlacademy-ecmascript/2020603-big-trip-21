...
import { transformToUpperCase } from '../../utils/common.js';
...
const renderTypesListTemplate = (offerTypes, type) => {
  const offerType = (offerTypes.length === 0) ? '' :
    offerTypes.map((item) => (
      `<div class="event__type-item">
        ...
        >
        <label class="event__type-label  event__type-label--${item.type}" for="event-type-${item.type}-1">${transformToUpperCase(item.type)}</label>
      </div>`)).join('');
      ...
      <form class="event event--edit" action="#" method="post">
        ...
        ${renderTypesListTemplate(pointOffers, type)}
          ...
          <div class="event__field-group  event__field-group--time">
            ...
            <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${humanizeDate(dateFrom, DateFormat.FULL_DATE)}">
            &mdash;
            <label class="visually-hidden" for="event-end-time-1">To</label>
            <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${humanizeDate(dateTo, DateFormat.FULL_DATE)}">
          </div>
